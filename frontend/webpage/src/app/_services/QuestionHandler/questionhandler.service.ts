...
export class QuestionhandlerService {
  ...
  retrievequestions(): Array<Question> {
    ...
    for(let x = 0; x < 10; x++) {
      ...
      ListQuestion.push(new Question(x, this.testquestion, this.asker_username_testlist[x], Math.floor(Math.random() * 1000), Math.floor(Math.random() * 60)));
  ...
  RetrieveQuestionsAnsweredbyUser(username: String): Array<Question> {
    ...
    for(let x = 0; x < 10; x++) {
      ArrayofQuestion.push(new Question(x, this.testquestion, this.asker_username_testlist[x], Math.floor(Math.random() * 1000), Math.floor(Math.random() * 60)));
    }
